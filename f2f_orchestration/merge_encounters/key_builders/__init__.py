"""Merge key builders: one small class per merge-encounters topic.

``BUILDERS`` fixes the order of topics under ``results`` and is what
:class:`MergeEncountersEngine` iterates.
"""
...
# Order here is the order of topics under ``results`` in merge-encounters/results.json.
BUILDERS: tuple[MergeKeyBuilder, ...] = (
