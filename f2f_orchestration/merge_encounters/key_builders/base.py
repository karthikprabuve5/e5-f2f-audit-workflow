"""Shared contract and helpers for merge key builders.

Each merge-encounters topic (``timely_encounter``, ``primary_hh_reason``, ...) is
produced by one small :class:`MergeKeyBuilder`. A builder returns a
``{poc_485, f2f_encounters}`` object; the engine assembles them under ``results``.
Builders are pure functions of ``TransactionOutputs`` + :class:`EvidenceResolver` —
no I/O, no verdicts.
...
@runtime_checkable
class MergeKeyBuilder(Protocol):
    """One merge-encounters topic. ``key`` is the top-level name under ``results``."""
...
def build_f2f_encounters(
    ...
    indices aligned across every merge key and never silently drops an encounter.
    """
...
class ReasoningOnlyBuilder:
    ...
    ``reasoning`` is ``{status?, summary, missing, evidence}``. Encounters the agent
    did not run (e.g. conditional telehealth / surgical, or a failure) get null
    values. Subclasses set ``key`` and ``agent_name``, and may set
    ``include_status = True`` to surface the agent's ``reasoning.status`` inside the
    reasoning block (kept consistent with the clinical-pillar builders).
    """
        ...
        return {
            ...
            "f2f_encounters": build_f2f_encounters(
                ...
                lambda index, output: {
                    ...
                    "reasoning": reasoning_block(output, resolver, include_status=include_status),
                },
                lambda index: {
                    "encounter_index": index,
                    "status": None,
                    "confidence": None,
                    "reasoning": null_reasoning(include_status=include_status),
                },
            ),
        }
