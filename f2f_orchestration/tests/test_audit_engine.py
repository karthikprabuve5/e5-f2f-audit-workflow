"""Unit tests for the audit engine (pure ``audit-results`` assembly).
...
from f2f_orchestration.audit import AuditEngine, TransactionOutputs
...
def audit() -> dict[str, Any]:
    ...
    return AuditEngine().build(outputs, generated_at=_GENERATED_AT)
...
def test_build_is_deterministic() -> None:
    outputs = TransactionOutputs.from_mapping(_payload())
    first = AuditEngine().build(outputs, generated_at=_GENERATED_AT)
    second = AuditEngine().build(outputs, generated_at=_GENERATED_AT)
    assert first == second
