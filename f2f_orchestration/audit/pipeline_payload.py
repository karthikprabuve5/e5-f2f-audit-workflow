"""Adapt live pipeline result dicts into the audit engine's ``from_mapping`` payload.

The POC and F2F pipelines accumulate their outputs in a :class:`ResultStore`, whose
``results`` dict is shaped for streaming/persistence — not for the audit engine.
:func:`build_audit_payload` performs the small, pure reshape that bridges the two,
so an in-process orchestrator can go pipeline → audit without touching disk:

    payload = build_audit_payload(poc_store.results, f2f_store.results,
                                  transaction_id=txn, client_id=client)
    audit = AuditEngine().build(TransactionOutputs.from_mapping(payload), generated_at=now)
...
def _transpose_encounters(
    ...
    The audit engine keys per-encounter agent outputs by agent first; the store keys
