...
description: >-
  ...
  references, encounter type classification using a 15-category taxonomy, and
...
compatibility: >-
  ...
  /skills/classification/classification/references/
---
...
typed clinical encounters. Each encounter is classified using a strict 15-category
...
6. Referral/intake document ("Reason for Referral", "Refer to PT/OT/SLP/SN", referral order/letter, HH intake) → `referral_documents / 15.x` (over `physician_orders`); a referral embedded in a discharge summary stays `9.1`, and one within the 485 stays `poc_485`
7. Explicit Addendum → `addendum / ADDENDUM` (set `parent_encounter_index`)
8. `UNPLACED` — only when classification cannot be made with confidence
