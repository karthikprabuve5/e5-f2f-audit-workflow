"""Assemble the ``merge_encounters`` contract from one transaction's outputs.

:class:`MergeEncountersEngine` is the pure core: given a :class:`TransactionOutputs`
and a ``generated_at`` timestamp, it emits the stable envelope, runs every key builder
in registry order, and derives a ``data_quality`` section. It performs no I/O, reads no
...
from .key_builders import BUILDERS, MergeKeyBuilder
...
class MergeEncountersEngine:
    """Builds the consolidated ``merge_encounters`` dict for one transaction."""

    def __init__(
        self,
        *,
        schema_version: str = _DEFAULT_SCHEMA_VERSION,
        builders: Sequence[MergeKeyBuilder] = BUILDERS,
    ...
    def build(self, outputs: TransactionOutputs, *, generated_at: str) -> dict[str, Any]:
        """Return the full ``merge_encounters`` dict. ``generated_at`` is injected (ISO 8601)."""
